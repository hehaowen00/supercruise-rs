...
impl<T> Node<T> {
    ...
    fn add_node_static(&mut self, p: &str) -> &mut Self {
    ...
    fn add_node_dynamic(&mut self, c: char, kind: NodeKind) -> &mut Self {
        self.add_node(c, kind)
    }

    /// Inserts a path into node.
    fn insert(&mut self, p: &str) -> &mut Self {
        match self.kind {
    ...
    fn find<'a>(&'a self, p: &'a str) -> Option<(&'a Self, Vec<&'a str>)> {
        self.find_with_capacity(p, 10)
    }

    /// Returns a reference to the node corresponding to the path.
    fn find_with_capacity<'a>(
        &'a self,
        p: &'a str,
        capacity: usize,
...
impl<T> PathTree<T> {
    ...
    fn insert(&mut self, mut path: &str, data: T) -> &mut Self {
        let mut next = true;
    ...
    fn find<'a>(&'a self, path: &'a str) -> Option<(&'a T, Vec<(&'a str, &'a str)>)> {
        self.root
...
fn routing_test() {
    ...
    tree.insert("api/user/:id", 5);
    tree.insert("api/users", 0);
    tree.insert("users", 1);
    println!("{:#?}", tree);
}
